...
struct ft_http_client_t
{
	...
	enum content_encoding_e content_encoding;
...
#define close_socket(fd)           \
	if (fd != HTTP_INVALID_SOCKET) \
	{                              \
		socket_close(fd);          \
		fd = HTTP_INVALID_SOCKET;  \
	}
#define close_file(pf) \
	if (pf != NULL)    \
	{                  \
		fclose(pf);    \
		pf = NULL;     \
...
static int socket_select(ft_http_client_t *http, int mode, int timeout)
{
	...
	while (remaind > 0)
	{
		...
		if (mode & kSelectWrite)
		{
			FD_ZERO(&wfd);
			FD_SET(http->fd, &wfd);
		}
		if (mode & kSelectError)
		{
			FD_ZERO(&efd);
			FD_SET(http->fd, &efd);
		}
...
static int parser_field_value(ft_http_client_t *http)
{
		...
		http->header_field && http->header_value)
	{
		...
		else if (_stricmp(http->header_field, "Content-Encoding") == 0)
		{
			printf("header %s:%s\n", http->header_field, http->header_value);
			if (_stricmp(http->header_value, "gzip") == 0)
			{
				http->content_encoding = CONTENT_ENCODING_GZIP;
			}
		}
		else
		{
...
int decode_gzip(Byte *zdata, uLong nzdata,
...
HTTP_API const char *ft_http_sync_request(ft_http_client_t *http, const char *url, http_request_method_e m)
{
	...
	if (http->content_encoding == CONTENT_ENCODING_GZIP)
	{
		...
		memcpy(&textLen, &http->body[http->body_len - 4], 4);
		http->bodyText = (char *)malloc(textLen + 1);

		decode_gzip((Byte *)http->body, http->body_len, (Byte *)http->bodyText, &textLen);
	}
	else
	{
		http->bodyText = (char *)malloc(http->body_len + 1);
		memcpy(http->bodyText, http->body, http->body_len);
	}
